...
const DailyMenuSkeleton = () => (
  <section id="daily-menu" className="mb-16 scroll-mt-28">
                ...
                <div key={j} className="flex items-start gap-4">
                  ...
                  <div className="flex-grow space-y-2">
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3].map((k) => (
                        <div key={k} className="h-7 w-7 bg-gray-200 rounded-full animate-pulse"></div>
                      ))}
                    </div>
                  </div>
                </div>
    ...
    <div className="-mx-4 sm:-mx-6 lg:-mx-8">
      <footer className="bg-gray-900 text-white p-5 px-4 sm:px-6 lg:px-8">
      ...
      <header className="sticky top-0 z-40 w-full bg-white">
        <div className="flex h-20 max-w-5xl mx-auto items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href={`/menu/${restaurant.slug}`}>
            <Image src="/icon_alergenu.png" alt="Alergenu Logo" width={36} height={36} className="flex-shrink-0" priority />
          </Link>
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            >
              <Image src="/icons/web_icons/filtro_alergeno.svg" alt="Filtro" width={18} height={18} className="mr-2" priority />
              {staticTexts.editAllergies[lang]}
              {/* Always render badge to reserve space, use opacity for visibility */}
              <span
                className={`absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#EA3939] text-xs font-bold text-white transition-opacity duration-200 ${hasMounted && selectedAllergens.length > 0 ? 'opacity-100' : 'opacity-0'
                  }`}
              >
